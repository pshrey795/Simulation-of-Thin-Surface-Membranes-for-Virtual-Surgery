...
{
    unsigned int n;
    vector<T> invdiag;              // reciprocals of diagonal elements
    vector<T> value;                // values below the diagonal, listed column by column
    vector<unsigned int> rowindex;  // a list of all row indices, for each column in turn
    vector<unsigned int> colstart;  // where each column begins in rowindex (plus an extra entry at the end, of #nonzeros)
    vector<T> adiag;                // just used in factorization: minimum "safe" diagonal entry allowed
...
template<class T>
void factor_modified_incomplete_cholesky0(const SparseMat<T> &matrix, SparseColumnLowerFactor<T> &factor, T modification_parameter=0.97, T min_diagonal_ratio=0.25){
    // first copy lower triangle of matrix into factor (Note: assuming A is symmetric of course!)
    factor.resize(matrix.n);
    fill(factor.invdiag.begin(), factor.invdiag.end(), 0); // important: eliminate old values from previous solves!
    factor.value.resize(0);
    factor.rowindex.resize(0);
    fill(factor.adiag.begin(), factor.adiag.end(), 0);
...
struct PCGSolver {
    ...
    bool solve(const SparseMat<T> &matrix, const std::vector<T> &rhs, std::vector<T> &result, T &residual_out, int &iterations_out) {
    ...
    void form_preconditioner(const SparseMat<T>& matrix){
