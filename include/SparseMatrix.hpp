...
//DONT IMPORT bits/stdc++ since there may already be an implementation of Sparse Matrix which 
...
template<class T>
struct SparseMat {
    unsigned int n;                             // dimension(number of rows)
    vector<vector<unsigned int>> index;         // for each row, a list of all column indices (sorted)
    vector<vector<T>> value;                    // values corresponding to index

    explicit SparseMat(unsigned int n_=0, unsigned int expected_nonzeros_per_row=7) : n(n_), index(n_), value(n_){
...
typedef SparseMat<float> SparseMatrixf;
typedef SparseMat<double> SparseMatrixd;

// Matrix-Vector Multiplication (result = matrix * x)
template<class T>
void multiply(const SparseMat<T> &matrix, const vector<T> &x, vector<T> &result){
...
// Update step using Matrix-Vector Multiplication (result -= matrix * x)
template<class T>
void multiply_and_subtract(const SparseMat<T> &matrix, const std::vector<T> &x, std::vector<T> &result){
    assert(matrix.n==x.size());
    result.resize(matrix.n);
    for(unsigned int i=0; i<matrix.n; ++i){
        for(unsigned int j=0; j<matrix.index[i].size(); ++j){
...
//============================================================================
// Fixed version of SparseMat. This is not a good structure for dynamically
...
struct FixedSparseMatrix
{
    ...
    vector<T> value;                                    // nonzero values row by row
    vector<unsigned int> colindex;                      // corresponding column indices
    vector<unsigned int> rowstart;                      // where each row starts in value and colindex (and last entry is one past the end, the number of nonzeros)
    ...
    void construct_from_matrix(const SparseMat<T> &matrix){
        resize(matrix.n);
        rowstart[0]=0;
        for(unsigned int i=0; i<n; ++i){
            rowstart[i+1] = rowstart[i]+(unsigned int)matrix.index[i].size();
        }
        //rowstart[n] stores the number of non-zero values in the matrix 
